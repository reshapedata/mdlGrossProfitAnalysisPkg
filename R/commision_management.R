...
a.FCommisionAmountUsd as 总计佣金金额_美元
 from rds_t_commision_res_mngr a
")
...
#'
#' @examples
#' commision_management_insert()
commision_management_insert<- function(dmstoken,year,MONTH) {

  sql=paste0("insert into rds_t_commision_res_mngr
select * from rds_vw_commision_fiscal_sum_mngr
...
#'
#' @examples
#' commision_management_delete()
commision_management_delete<- function(dmstoken) {

  sql=paste0("
TRUNCATE TABLE rds_t_commision_res_mngr")
...
a.FCommisionAmountUsd as 总计佣金金额_美元
 from rds_t_commision_res_mngr a
             where year(a.FDate)='",year,"' and MONTH(a.FDate)='",MONTH,"'")
  res=tsda::sql_select2(token = dmstoken,sql = sql)
  return(res)
}
...
#'
#' @examples
#' commision_management_deleteBydate()
commision_management_deleteBydate<- function(dmstoken,year,MONTH) {

  sql=paste0("delete from rds_t_commision_res_mngr
